setwd("~/Dropbox (UMass Medical School)/Project/Junhui_Li_UMMS/Michael_Green/Shahid_Banday/20230309_kaplan_meier/result/gse63157_trans/")


##------------------
## prepare the dataset
##------------------
gse63157_trans_raw <- read.table("gse63157_transcript.txt",comment.char = "",header=T)

# select specified rows for next step
gse63157_trans_C1GALT1 <- gse63157_trans_raw[c(1:16,which(gse63157_trans_raw[,1] %in% c("C1GALT1","SMO","GLI1"))),]
gse63157_trans_3gene <- as.data.frame(t(gse63157_trans_C1GALT1))

# select specified columns for next step
gse63157_trans <- gse63157_trans_3gene[,c(4:6,9,10,14,15,17:19)]
colnames(gse63157_trans) <- c("efs_time","efs_status","efs_event","os_time","os_status","survival_status","survival_time","C1GALT1","GLI1","SMO")

# remove unused rows
dat <- gse63157_trans[-c(1:2),]
...
pdf(paste0("correlation/gse63157_trans_C1GALT1_GLI1_correlation.pdf"))
print(pc1)
dev.off()
pdf(paste0("correlation/gse63157_trans_C1GALT1_SMO_correlation.pdf"))
print(pc2)
dev.off()
# install.packages("PerformanceAnalytics")
library(PerformanceAnalytics)
dat_corr_gene <- dat_corr[,c("C1GALT1_log2","GLI1_log2","SMO_log2")]
pdf(paste0("correlation/gse63157_trans_C1GALT1_GLI1_SMO_correlation.pdf"))
...
cutoff <- list()
cutoff["scan"]  <- list(55)
cutoff["median"]  <- list(43)
cutoff["average"]  <- list(50)
cutoff["first_quartile"]  <- list(22)
cutoff["last_quartile"]  <- list(64)
cutoff["first_vs_last_quartile"]  <- list(22)
cutoff["curtain"]  <- list(34)

cutoff_ef <- list()
cutoff_ef["scan"]  <- list(49)
cutoff_ef["median"]  <- list(43)
cutoff_ef["average"]  <- list(50)
cutoff_ef["first_quartile"]  <- list(22)
cutoff_ef["last_quartile"]  <- list(64)
cutoff_ef["first_vs_last_quartile"]  <- list(22)
cutoff_ef["curtain"]  <- list(33)
...
for(i in names(cutoff)){
  ...
  #(p1$plot/p1$table)|p2
  pdf(paste0("kaplan_meier_curve/gse63157_trans_2plots_cutoff_method_",i,".overall.pdf"))
  ...
  BBBBBBCC
  "
  pdf(paste0("kaplan_meier_curve/gse63157_trans_3plots_cutoff_method_",i,".overall.pdf"))
...
for(i in names(cutoff_ef)){
  ...
  #(p1$plot/p1$table)|p2
  pdf(paste0("kaplan_meier_curve/gse63157_trans_2plots_cutoff_method_",i,".eventfree.pdf"))
  plot_layout(widths = c(2, 1), heights = c(1, 1))
  print(p1_efs$plot + p2 +  plot_layout(widths = c(3, 1)))
  dev.off()
  pdf(paste0("kaplan_meier_curve/gse63157_trans_3plots_cutoff_method_",i,".eventfree.pdf"))
